 ...
 * Their mixins only target vanilla's End source, so WoVer must consume the same registry.
 */
    ...
    public static int importRegisteredBiomes(Registry<Biome> biomes) {
        ...
        for (String registryClassName : REGISTRY_CLASSES) {
            try {
                ...
                // The adapter is optional and must not make either mod a required dependency.
