...
public final class IncisionBiomeSourceCompat {
    private static final ResourceKey<Biome> ERODED_YARD = ResourceKey.create(
            Registries.BIOME,
    ...
    public static BiomeSource prepare(BiomeSource source) {
        ...
        if (access == null || access.registryOrThrow(Registries.BIOME).getHolder(ERODED_YARD).isEmpty()) {
            return source;
        }
    ...
    private static Climate.ParameterList<Holder<Biome>> getParameters(MultiNoiseBiomeSource source)
        ...
        for (Method method : source.getClass().getDeclaredMethods()) {
            if (method.getParameterCount() == 0
                    && Climate.ParameterList.class.isAssignableFrom(method.getReturnType())) {
