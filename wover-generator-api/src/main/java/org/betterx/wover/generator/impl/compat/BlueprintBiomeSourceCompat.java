...
public final class BlueprintBiomeSourceCompat {
    ...
    private static final ResourceKey<? extends Registry<?>> SLICES_REGISTRY = ResourceKey.createRegistryKey(
            ResourceLocation.fromNamespaceAndPath("blueprint", "modded_biome_slices")
    );
    ...
    public static void importActiveEndOverlays(RegistryAccess access, Registry<Biome> biomes) {
        ...
        try {
            ...
            for (Object slice : slices) {
                ...
                if (weight <= 0 || !(levels instanceof Set<?> set) || !set.contains(LevelStem.END)) {
                    continue;
                }

                Object provider = invoke(slice, "provider");
                if (provider.getClass().getName().endsWith("BiomeUtil$OriginalModdedBiomeProvider")) {
                    continue;
                }
                ...
                if (!(overlays instanceof Iterable<?> iterable)) {
                    continue;
                }

                for (Object overlayObject : iterable) {
                    if (!(overlayObject instanceof Pair<?, ?> overlay)
                            || !(overlay.getSecond() instanceof BiomeSource overlaySource)) {
                        continue;
                    }
                    ...
                    for (Holder<Biome> holder : overlaySource.possibleBiomes()) {
                        ...
                        if (key == null || !biomes.containsKey(key)) {
                            continue;
                        }

                        tagWorker.addBiomeToTag(tag, biomes, key, holder);
                        imported.putIfAbsent(key, new WoverBiomeData(
                                1.0F,
                                key,
        ...
        if (!imported.isEmpty()) {
            LibWoverWorldGenerator.C.log.info("Imported {} active Blueprint End biome overlay(s)", imported.size());
        }
    }

    private static TagKey<Biome> classifyEndOverlay(Object matchesObject) {
        if (!(matchesObject instanceof Iterable<?> matches)) {
            return null;
        }

        boolean highlands = false;
        boolean barrens = false;
        boolean islands = false;
        boolean center = false;
        for (Object match : matches) {
            if (!(match instanceof Holder<?> holder)) {
                continue;
            }
        ...
        if (barrens) return CommonBiomeTags.IS_END_BARRENS;
        return null;
    }
